...
pub(crate) fn sa_check_roles_and_impl(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    expand_checked_fn(
        ...
        quote! {
            #login_id
            sa_token_core::StpUtil::check_all_roles(&__sa_login_id, &[#(#role_lits),*]).await?;
        },
    )
}
