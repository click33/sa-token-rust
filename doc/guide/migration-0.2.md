...
| `put_stp_logic` / global registry | `SaLogic::new` / `StpUtil::stp_logic` |
...
Prefer `try_build` / `try_init_manager` / `try_get_manager`. Adapters should read tokens with `token_io::read_token`. Login and grants go through `AuthService` / `AuthzService`. Multi-account uses `login_type` + `SaLogic`.

Real new paths include: `dao.rs`, `keys.rs`, `token_io.rs`, `service/`, `stp_logic.rs`, `oauth2/`, `sso/`, `cleanup/`, and `sa-token-plugin-common`. Dao has **no** `set_json`; configuration errors use `ConfigError`.
...
9. Replace `put_stp_logic` with `SaLogic::new` / `StpUtil::stp_logic`.
10. Run `cargo check --workspace`, then `cargo clippy --workspace --lib`.
